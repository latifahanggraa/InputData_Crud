...
if(empty($foto)){ // Jika user tidak memilih file foto pada form
  ...
  $sql = $pdo->prepare("UPDATE siswa SET nama=:nama, nis=:nis, jenis_kelamin=:jenis_kelamin, jurusan=:jurusan WHERE id=:id");
  ...
  if(move_uploaded_file($tmp, $path)){ // Cek apakah gambar berhasil diupload atau tidak
    ...
    $sql = $pdo->prepare("SELECT foto FROM tb_ukk WHERE id=:id");
    $sql->bindParam(':id', $id);
    ...
    // Proses ubah data ke Database
    $sql = $pdo->prepare("UPDATE tb_ukk SET nama=:nama, nis=:nis, jenis_kelamin=:jenis_kelamin, jurusan=:jurusan, foto=:foto WHERE id=:id");
    $sql->bindParam(':nama', $nama);
    $sql->bindParam(':nis', $nis);
    $sql->bindParam(':jenis_kelamin', $jenis_kelamin);
    $sql->bindParam(':jurusan', $jurusan);
